...
% Huffman coding using Horizontal DPCM
[countsH_nz, symbolsH_nz, ~] = build_counts_and_lut(DPCM_H(:), 510);
probH_nz = countsH_nz / sum(countsH_nz);
dictH    = huffmandict(symbolsH_nz, probH_nz);
stream_huff_H = huffmanenco(DPCM_H(:), dictH);
bits_huffman_DPCM_H   = numel(stream_huff_H);
...
% Huffman coding using Vertical DPCM
[countsV_nz, symbolsV_nz, ~] = build_counts_and_lut(DPCM_V(:), 510);
probV_nz = countsV_nz / sum(countsV_nz);
dictV    = huffmandict(symbolsV_nz, probV_nz);
stream_huff_V = huffmanenco(DPCM_V(:), dictV);
bits_huffman_DPCM_V   = numel(stream_huff_V);

% Arithmetic using Horizontal DPCM
[~, ~, lutV] = build_counts_and_lut(DPCM_V(:), 510);
seqV = lutV(double(DPCM_V(:))+1);
stream_arith_V = arithenco(double(seqV), countsV_nz);
bits_arithmetic_DPCM_V   = numel(stream_arith_V);
...
print_row('DPCM-Hor', RAW_BITS, bits_huffman_DPCM_H,    bits_arithmetic_DPCM_H);
print_row('DPCM-Ver', RAW_BITS, bits_huffman_DPCM_V,    bits_arithmetic_DPCM_V);
fprintf('=========================================================\n');
